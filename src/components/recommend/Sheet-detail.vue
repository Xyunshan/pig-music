...
import { getSongsheetDetail } from "@/api/getSongsheet.js";
// 格式化歌曲列表
...
export default {
  ...
  methods: {
    ...
    _getSongsheetDetail() {
      ...
      getSongsheetDetail(this.dedail.id).then(res => {
        if (res.code === 0) {
          this.playNum = res.cdlist[0].visitnum;
